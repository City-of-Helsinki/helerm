...
class FunctionQuerySet(models.QuerySet):
    def latest_version(self):
        return self.order_by("classification__code", "-version").distinct(
...
class Function(StructuralElement):
    ...
    name = models.CharField(
        verbose_name=_("name"), max_length=256, blank=True
    )  # only templates use this field
    ...
    _attribute_validations = {
        "allowed": (
            ...
            "PublicityClass",
            "ProtectionClass",
            "Restriction.ProtectionLevel",
            "Restriction.SecurityClass",
            "Restriction.SecurityPeriodStart",
            "RetentionPeriod",
            "RetentionPeriodOffice",
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.id:
            ...
            try:
                latest = Function.objects.latest_version().get(
                    classification__uuid=self.classification.uuid
                )

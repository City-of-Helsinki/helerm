...
class ClassificationAdmin(admin.ModelAdmin):
    ...
    def create_new_version_view(self, request, object_id):
        ...
        latest_version = Classification.objects.latest_version().get(uuid=classification.uuid)
        latest_version.pk = None  # New version will be created if pk is none
        latest_version.state = Classification.DRAFT
        latest_version.save()

        return HttpResponseRedirect(
            reverse('admin:metarecord_classification_change', kwargs={"object_id": latest_version.pk})
        )

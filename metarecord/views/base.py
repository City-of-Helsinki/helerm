...
class StructuralElementSerializer(serializers.ModelSerializer):
    ...
    def get_fields(self):
        ...
        fields = {
            ...
            or StructuralElement.can_view_modified_by(request.user)
        }
    ...
    def get_valid_attribute_dict(self):
        if not hasattr(self, "_valid_attribute_dict"):
            self._valid_attribute_dict = {
                attr.identifier: attr for attr in Attribute.objects.all()
            }
    ...
    def get_attribute_validation_errors(self, instance, recursive=True):  # noqa: C901
        ...
        allow_values_outside_choices_attributes = (
            instance.get_allow_values_outside_choices_attributes()
        )
        ...
        for (
            ...
            if (
                condition_attribute in instance.attributes
                and instance.attributes[condition_attribute] in value
            ):
        ...
        required_attributes = (
            required_attributes & valid_attribute_dict.keys()
        ...
        for attribute, value in instance.attributes.items():
            ...
            if (
                not instance.is_attribute_allowed(attribute)
                or attribute in disallowed_attributes
            ):
            ...
            if isinstance(value, list) and attribute not in multivalued_attributes:
                attribute_errors[attribute].append(
                    _("This attribute does not allow multiple values.")
                )
                continue
            ...
            for one_value in value:
                ...
                if (
                    one_value not in allowed_values
                    and attribute not in allow_values_outside_choices_attributes
                ):
        ...
        if recursive and child_relation_name:
            ...
            child_objects = (
                getattr(instance, child_relation_name).all().order_by("index")
            )
            child_errors = [
                self.get_attribute_validation_errors(child_obj)

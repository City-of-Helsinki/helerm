...
def test_refactor_conditional_validation():
    ...
    refactor_conditional_validation(
        attr_validations, refactored_validations, "conditionally_required"
    )
...
def test_structural_element_multivalued_attributes_with_defined_validation(function):
    ...
    assert all(
        [prop in multivalued_attributes for prop in function_multivalued_attributes]
    )


@pytest.mark.django_db
def test_structural_element_conditionally_required_attributes_with_defined_validation(
    function,
):
    # Function is a subclass of StructuralElement
    content_type = ContentType.objects.get_for_model(function)
    ...
    function_conditionally_required_attributes = (
        function.get_conditionally_required_attributes()
    )

    assert len(function_conditionally_required_attributes) == len(
        conditionally_required_attributes
    )


@pytest.mark.django_db
def test_structural_element_conditionally_disallowed_attributes_with_defined_validation(
    function,
):
    # Function is a subclass of StructuralElement
    content_type = ContentType.objects.get_for_model(function)
    ...
    function_conditionally_disallowed_attributes = (
        function.get_conditionally_required_attributes()
    )

    assert len(function_conditionally_disallowed_attributes) == len(
        conditionally_disallowed_attributes
    )
...
@pytest.mark.django_db
def test_structural_element_allow_values_outside_choices_attributes_with_defined_validation(
    function,
):
    # Function is a subclass of StructuralElement
    content_type = ContentType.objects.get_for_model(function)
    ...
    function_allow_values_outside_choices = (
        function.get_allow_values_outside_choices_attributes()
    )

    assert all(
        [
            prop in allow_values_outside_choices
            for prop in function_allow_values_outside_choices
        ]
    )

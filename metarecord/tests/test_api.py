...
def test_attribute_get_list_and_detail(choice_attribute, choice_value_1, attribute_group, user_api_client):
    for url in (ATTRIBUTE_LIST_URL, get_attribute_detail_url(choice_attribute)):
        ...
        assert value_datum['description'] == choice_value_1.description
...
def test_attribute_endpoints(user_api_client, choice_attribute, choice_value_1, attribute_group, endpoint):
    ...
    assert value_data.keys() == {'id', 'value', 'created_at', 'modified_at', 'index', 'name', 'description'}
...
@pytest.mark.parametrize('authenticated', (False, True))
@pytest.mark.django_db
def test_function_visibility_in_classification(api_client, user_api_client, classification, authenticated):
    client = user_api_client if authenticated else api_client
...
@pytest.mark.django_db
def test_function_version_filter(api_client, user_api_client, classification, authenticated, index):
    client = user_api_client if authenticated else api_client
